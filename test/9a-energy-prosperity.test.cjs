...
test('9A preserves source snapshot, units and all in-range observations',()=>{
 ...
 for(const d of source){assert.ok(p.pointX(d.energy)>=130&&p.pointX(d.energy)<=1120);assert.ok(p.pointY(d.gdp)>=220&&p.pointY(d.gdp)<=600);assert.ok(((p.pointX(d.energy)-340)/140)**2+((p.pointY(d.gdp)-350)/100)**2>1,'highlighted ellipse contains no observations');}
});
