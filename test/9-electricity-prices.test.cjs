...
test('all eleven source bars and render registration are present',()=>{
 ...
 const entry=JSON.parse(fs.readFileSync(path.join(root,'_render/charts.json'))).find(c=>c.id==='9b');
 assert.equal(entry.duration,DURATION);assert.equal(entry.html,'9b-electricity-prices.html');
});
