...
await page.goto(base+'/9b-electricity-prices.html');await page.locator('[data-seek="12500"]').click();assert.equal(await page.locator('#scrubber').inputValue(),'12500');await page.locator('#replayBtn').click();assert.equal(await page.getByRole('button',{name:'Render .mov',exact:true}).count(),1);
