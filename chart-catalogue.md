...
- **Data source:** No source SVG exists for this one — Kevin provided only a reference PNG and a one-paragraph script excerpt. The map illustration (terrain blobs, river bend, flood band, street-grid texture inside the urban ring) is an original recreation built to visually match the reference, not a data extraction — flagged explicitly since every other chart in this series has had an exact vector/data source. Ring radii per year are estimated proportionally from the reference image, not measured.
