...
Design system (applies to all charts unless noted): dark navy background (~#032C4E), Bright Turquoise `#04FFBA` primary data color, Vivid Blue `#043FC5` secondary accent, Rajdhani font. Export: transparent background, transparent rounded corners, ProRes 4444 alpha `.mov` (`prores_videotoolbox`, `-pix_fmt bgra`), 1920x1200, 30fps. Runs ~1.1-1.9GB per 8-13s clip. Two smaller alternatives (HEVC-with-alpha via `avconvert`, GoPro CineForm) were tried on 2026-08-21/24 and both failed real After Effects import despite passing every automated/AVFoundation-level test — see the `chart-hevc-alpha-encoding` memory before attempting another codec change here.
