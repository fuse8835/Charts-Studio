...
- **Map detail pass (2026-08-26):** rebuilt after Kevin sent a close-up of the 2040 panel and asked for more fidelity — the river now forks (one trunk splitting into two branches right where the city sits, matching the close-up exactly) instead of a single bend, and the urban texture is real street-grid lines + building rectangles instead of a plain dot pattern. Per Kevin's own suggested approach: the building/street layout is generated **once** (a seeded deterministic PRNG, so it's reproducible, not random per render) and reused via SVG `<use>` in all four panels, each just clipped to that year's exurban radius — so all four panels show literally the same underlying city, with smaller years simply revealing less of it. Building/street density and the river fork position are estimated from the reference, not measured.
